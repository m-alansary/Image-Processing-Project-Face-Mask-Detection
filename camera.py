...
import cv2
import os
from studentsData import add_student, currentId
    ...
    if name.isalpha():
        ...
        while(True):
            ...
            for (p1, p2, p3, p4) in faces:
                cv2.rectangle(image, (p1, p2), (p1 + p3, p2 + p4), (0, 0, 255), 2) # BGR
                imageNo += 1
                cv2.imwrite("Training Images" + os.sep + name + "." + str(currentId) + '.' +
        ...
        cam.release()
        cv2.destroyAllWindows()
        res = "Training data is saved for ID: " + str(currentId) + " and Name: " + name
        row = [currentId, name]
        add_student(name)
